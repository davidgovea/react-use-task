...
test('concurrency: 1', async t => {
  ...
  const mapper = ([value, ms]) =>
    limit(() => delay(ms).mapResult(() => value));
...
test('concurrency: 4', async t => {
	...
	const input = Array.from({length: 100}, () => limit(() => {
    running++;
...
test('continues after sync throw', async t => {
	...
	await Future.all(futures).mapError(() => undefined).toPromise();
...
test('does not ignore errors', async t => {
	...
	const futures = [
		limit(() => delay(30)),
		limit(() => delay(80).map(() => { throw error; })),
		limit(() => delay(50)),
	];
...
test('activeCount and pendingCount properties', async t => {
	...
	const immediateFutures = Array.from({length: 5}, () => limit(() => delay(1000)));
	const delayedFutures = Array.from({length: 3}, () => limit(() => delay(1000)));
